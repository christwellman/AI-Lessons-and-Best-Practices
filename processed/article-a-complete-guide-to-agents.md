...
tags: [agents-md, context-window, prompt-engineering, progressive-disclosure]
summary: "Large instruction files waste tokens and confuse coding agents. Keep the main configuration file small and use progressive disclosure for detailed rules. Place specific guidance in nested files or separate documents."
...
Large instruction files waste tokens and confuse coding agents. Keep the main configuration file small and use progressive disclosure for detailed rules. Place specific guidance in nested files or separate documents.

## Key Takeaways

- Frontier LLMs follow about one hundred fifty to two hundred instructions with reasonable consistency.
- Every token in the main configuration file loads on every single request.
- Stale documentation poisons the agent context and causes errors.
- The ideal configuration file contains only a project description, package manager, and critical commands.
- Use nested files and references to load language rules only when necessary.
- Place configuration files in subdirectories for monorepos to merge context.

## Techniques / Prompts Extracted

- ln -s CLAUDE.md AGENTS.md
- We are building a React Native mobile app for tracking plant watering schedules.
- Use pnpm as the package manager for this repo.
- For TypeScript conventions, see docs/typescript.md
- For TypeScript guidelines, read docs/typescript.md when working on .ts files
- See packages/api/AGENTS.md for API-specific guidelines
- This is the API package for our monorepo. It uses Express and PostgreSQL.
- Use pnpm --filter api build to build this package.
- Refactor our AGENTS.md file to be as small and focused as possible. Move domain-specific rules (like TypeScript guidelines, testing instructions, and style guides) into separate markdown files in a docs/ directory, and replace them with short references in AGENTS.md. Keep only the core project description, package manager, and essential build commands in the root.
