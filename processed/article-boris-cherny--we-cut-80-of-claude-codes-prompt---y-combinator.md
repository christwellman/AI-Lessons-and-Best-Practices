...
ingested: "2026-08-23"
category: "techniques"
tags: [prompt-ablation, agentic-workflows, unhobbling-models, test-time-compute]
summary: "Modern language models require an empirical approach that starts by deleting system prompts and rebuilding instructions line by line only when failures occur. Developers must un-hobble agents by assigning harder high-level tasks, providing verification mechanisms, and utilizing dynamic workflows or routines to orchestrate thousands of agents. Evals remain necessary to measure capability, but they saturate quickly and require frequent updates as model intelligence accelerates."
...
Modern language models require an empirical approach that starts by deleting system prompts and rebuilding instructions line by line only when failures occur. Developers must un-hobble agents by assigning harder high-level tasks, providing verification mechanisms, and utilizing dynamic workflows or routines to orchestrate thousands of agents. Evals remain necessary to measure capability, but they saturate quickly and require frequent updates as model intelligence accelerates.

## Key Takeaways

- Delete system prompts and test model performance without instructions to perform ablations.
- Add system prompt instructions back only when the model repeatedly stumbles on the same task.
- Describe high-level tasks, guardrails, and exit criteria instead of writing over-specific step-by-step instructions.
- Provide the model with tools to verify its own work along the way so it does not get stuck.
- Use dynamic workflows and routines to orchestrate thousands of parallel agents for complex long-running tasks.

## Techniques / Prompts Extracted

- Claude Code simple equals one
- use a workflow
